...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const allowed = await prisma.purchase
    .withAuthorization({
      roqUserId,
  ...
  async function getPurchaseById() {
    const data = await prisma.purchase.findFirst(convertQueryToPrismaUtil(req.query, 'purchase'));
  ...
  async function deletePurchaseById() {
