...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getAdministrations() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const response = await prisma.administration
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'administration'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createAdministration() {
    ...
    const data = await prisma.administration.create({
      data: body,
    });
