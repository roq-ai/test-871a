...
import { getAdministrations } from 'apiSdk/administrations';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'product-owner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<AdministrationInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/administrations` : null),
    () => getAdministrations({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
...

    {
      ...
      path: isTenantUser ? `/administrations/view/${data?.[0]?.id}` : '/administrations',
